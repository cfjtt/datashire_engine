...
import com.eurlanda.datashire.engine.spark.mllib.regression.RidgeRegressionSquid
...
/**
  ...
  */
class TRidgeRegressionSquid extends TTrainSquid {

  private val log: Log = LogFactory.getLog(classOf[TRidgeRegressionSquid])
  setType(TSquidType.RIDGE_REGRESSION_SQUID)
  ...
  override def run(jsc: JavaSparkContext): Object = {
    ...
    val regression = new RidgeRegressionSquid()
    ...
    try {
      if (key > 0) {
        ...
        keyDataCellList.foreach { kyDataCell =>
          ...
          saveModel(conn, tableName, saveModelSql,groupKey, modelVersion,
            lassoRegressionOut._1, lassoRegressionOut._2 )
          //是否设置了系数squid
          if (hasCoeffSquid) {
      ...
      case e: Throwable => {
        ...
        log.error("TRidgeRegressionSquid 异常:" + errorMessage)
  ...
  private def saveModel(conn: java.sql.Connection, tableName: String, saveModelSql: String, key: String, version: Int,
    ...
    try {
      ...
      case e: Throwable => {
        val errorMessage = e.getMessage
        log.error("保存模型异常" + errorMessage, e)
        log.error(e.getStackTrace())
