 ...
 *
 */
public class LogExtractBuilder extends AbsTSquidBuilder implements TSquidBuilder {

	public LogExtractBuilder(BuilderContext ctx, Squid currentSquid) {
	...
	public List<TSquid> doTranslate(Squid squid) {
		...
		WebLogExtractSquid wes = (WebLogExtractSquid) squid;
		...
		for (ReferenceColumn c : columns) {
            TColumn tColumn ;
            if (c.getName().equals("time_taken")) {//todo delete 容错
                tColumn = new TColumn(StringUtils.trim(c.getName()), -c.getColumn_id(), TDataType.LONG);
            } else {
                tColumn = new TColumn(StringUtils.trim(c.getName()), -c.getColumn_id(), TDataType.sysType2TDataType(c.getData_type()));
            }
		...
		fs.setFileType(FileType.LOG.name());
